...
import { useStore, User, TransportEvent, StoreProvider, RoomState } from "./api";
...
export const Conference = () => {
  ...
  const [roomState, setRoomState] = useState<RoomState>({ id: "", name: "", Text: "" })
  const [user, setUser] = useState<User>();
  const refTransport = useRef<WebSocketTransport>();
  if (!refTransport.current) {
    refTransport.current = new WebSocketTransport(
      `ws://localhost:8080/${window.location.pathname.replace("/", "")}`
    );
  }
  const handleChangeRoomState = (e: React.SyntheticEvent) => {
    const target = e.target as HTMLInputElement
    setRoomState({ ...roomState, [target.name]: target.value })
  }
  ...
  return (
    <div style={{ width: "100%" }}>
      <div>
        ...
        <input onChange={handleChangeRoomState} name="id" value={roomState["id"]} placeholder="id" />
        <input onChange={handleChangeRoomState} name="name" value={roomState["name"]} placeholder="name" />
        <input onChange={handleChangeRoomState} name="Text" value={roomState["Text"]} placeholder="Text" />
